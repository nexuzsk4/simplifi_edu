...
def load_settings(
    ...
    resolved_provider = (
        provider
        or get_value("INSPIRER_LLM_PROVIDER")
        or get_value("LLM_PROVIDER", DEFAULT_PROVIDER)
        or DEFAULT_PROVIDER
    ).strip()
    resolved_model = (
        model
        or get_value("INSPIRER_LLM_MODEL")
        or get_value("LLM_MODEL", DEFAULT_MODEL)
        or DEFAULT_MODEL
    ).strip()
    raw_temperature = get_value("INSPIRER_LLM_TEMPERATURE") or get_value("LLM_TEMPERATURE", str(DEFAULT_TEMPERATURE))
    try:
    ...
    if resolved_provider == "gemini":
        api_key = get_value("INSPIRER_GEMINI_API_KEY") or get_value("GEMINI_API_KEY")
    elif resolved_provider == "openai":
        api_key = get_value("INSPIRER_OPENAI_API_KEY") or get_value("OPENAI_API_KEY")
    else:
        api_key = None
