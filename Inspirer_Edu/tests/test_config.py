...
from preset_inspirer_agent.config import default_env_path, load_settings
...
class ConfigTests(unittest.TestCase):
    ...
    def test_uses_inspirer_env_prefix(self):
        settings = load_settings(
            environ={
                "INSPIRER_LLM_PROVIDER": "openai",
                "INSPIRER_LLM_MODEL": "gpt-5-nano",
                "INSPIRER_LLM_TEMPERATURE": "0.2",
                "INSPIRER_OPENAI_API_KEY": "key",
            },
            env_file="missing.env",
        )
        self.assertEqual(settings.provider, "openai")
