...
class GeminiProvider:
    ...
    def __init__(self, model: str, api_key: str, temperature: float = 0.3) -> None:
        if not api_key:
            raise ValidationError("GEMINI_API_KEY is required for GeminiProvider")
        try:
    ...
    def _build_response_schema(self):
        question_schema = self._types.Schema(type=self._types.Type.STRING)
...
class OpenAIProvider:
    ...
    def __init__(self, model: str, api_key: str, temperature: float = 0.3) -> None:
        if not api_key:
            raise ValidationError("OPENAI_API_KEY is required for OpenAIProvider")
        try:
