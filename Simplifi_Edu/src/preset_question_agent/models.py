...
INPUT_FIELDS = CONTEXT_FIELDS + (OUTPUT_LANGUAGE_FIELD,)
INPUT_ALIASES = {"outputlanguage": OUTPUT_LANGUAGE_FIELD}
DEFAULT_OUTPUT_LANGUAGE = "Thai"
NO_DATA = "ไม่มีข้อมูล"
NO_DATA_MARKERS = {"", "-", "n/a", "na", "none", "null", "ไม่มี", "ไม่มีข้อมูล"}

REQUIRED_CATEGORY_IDS = ("misconceptions", "common_questions", "foundations")
...
class LearningContext:
    ...
    def from_mapping(cls, data: Mapping[str, Any]) -> "LearningContext":
        ...
        for field_name in CONTEXT_FIELDS:
            ...
            if value is None:
                normalized[field_name] = NO_DATA
                continue
            if isinstance(value, bool):
                raise ValidationError(f"{field_name} must be a string, number, empty, or null")
            if field_name == "year" and isinstance(value, (int, float)):
                value = str(value)
